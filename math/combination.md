...
void init(int _n,ll _p){
    fac[0]=invfac[0]=1;
    for(int i=1;i<=_n;++i) fac[i]=fac[i-1]*i%_p;
    for(int i=1;i<=_n;++i) invfac[i]=invfac[i-1]*qpow(i,_p-2,_p)%_p;
}
inline ll getC(int _n,int _m,ll _p){
    if(_m>_n) return 0;
    return fac[_n]*invfac[_m]%_p*invfac[_n-_m]%_p;
}
```
